...
fn test_template_matching_performance() {
    ...
    for algorithm in algorithms {
        ...
        println!("{}: {:.2}ms avg, {:.3} confidence, {:.1}% success", 
                 algorithm.name(), avg_time, avg_confidence, success_rate * 100.0);
        
        // Performance requirements
        assert!(avg_time < 500.0, 
               "Algorithm {} too slow: {:.2}ms", algorithm.name(), avg_time);
        assert!(success_rate > 0.8, 
               "Algorithm {} success rate too low: {:.1}%", algorithm.name(), success_rate * 100.0);
        assert!(avg_confidence > 0.0, 
               "Algorithm {} confidence too low: {:.3}", algorithm.name(), avg_confidence);
    }
}
...
fn test_orb_performance() {
    ...
    let orb_configs = vec![
        OpenCVORB::new().with_fast_threshold(10).with_max_features(100),
        OpenCVORB::new().with_fast_threshold(10).with_max_features(500),
        OpenCVORB::new().with_fast_threshold(5).with_max_features(500),
        OpenCVORB::new().with_fast_threshold(15).with_max_features(500),
    ];
    ...
    for orb in orb_configs {
        ...
        println!("ORB(thresh={}, max={}): {:.2}ms avg, {:.3} confidence, {:.1}% success", 
                 orb.fast_threshold, orb.max_features, avg_time, avg_confidence, success_rate * 100.0);
        
        // ORB performance requirements (more lenient due to complexity)
        assert!(avg_time < 2000.0, "ORB too slow: {:.2}ms", avg_time);
        assert!(success_rate > 0.6, "ORB success rate too low: {:.1}%", success_rate * 100.0);
    }
}
...
fn test_cross_algorithm_performance() {
    ...
    for _ in 0..iterations {
        ...
        for result in results {
            println!("{}: {:.2}ms, confidence: {:.3}, translation: ({:.1}, {:.1})", 
                     result.algorithm_used, result.processing_time_ms, 
                     result.confidence, result.translation.0, result.translation.1);
    ...
    for (name, time) in all_times {
        ...
        assert!(time >= 0.0, "Algorithm {} invalid time: {:.2}ms", name, time);
    }
}
...
fn test_accuracy_vs_performance_tradeoff() {
    ...
    let configs = vec![
        ("Fast", OpenCVORB::new().with_fast_threshold(20).with_max_features(100)),
        ("Balanced", OpenCVORB::new().with_fast_threshold(10).with_max_features(500)),
        ("Accurate", OpenCVORB::new().with_fast_threshold(5).with_max_features(1000)),
    ];
    ...
    for (config_name, orb) in configs {
        ...
        println!("{}: {:.2}ms, {:.3} confidence, {:.1}% success", 
                 config_name, avg_time, avg_confidence, success_rate * 100.0);
        
        // All configs should work
        assert!(success_rate > 0.5, "{} success rate too low", config_name);
        assert!(avg_time > 0.0, "{} should have positive processing time", config_name);
    }
}
...
fn test_image_size_scaling() {
    ...
    for size in sizes {
        ...
        println!("Size {}x{}: {:.2}ms, {:.3} confidence, {:.1}% success", 
                 size, size, avg_time, avg_confidence, success_rate * 100.0);
        ...
        assert!(avg_time < expected_max_time, 
               ...
               size, avg_time, expected_max_time);
...
fn test_memory_efficiency() {
    ...
    for algorithm in algorithms {
        ...
        for i in 0..5 {
            ...
            assert!(result.is_ok(), "Algorithm {} failed on iteration {}", algorithm.name(), i);
...
fn test_regression_detection() {
    ...
    for baseline in baselines {
        ...
        // Regression checks
        assert!(avg_time <= baseline.max_time_ms, 
               "{} performance regression: {:.2}ms > {:.2}ms", 
               baseline.algorithm, avg_time, baseline.max_time_ms);
        
        assert!(avg_confidence >= baseline.min_confidence, 
               "{} confidence regression: {:.3} < {:.3}", 
               baseline.algorithm, avg_confidence, baseline.min_confidence);
        
        assert!(success_rate >= baseline.min_success_rate, 
               "{} success rate regression: {:.3} < {:.3}", 
               baseline.algorithm, success_rate, baseline.min_success_rate);
        
        println!("{} passed regression test: {:.2}ms, {:.3} confidence, {:.1}% success", 
                 baseline.algorithm, avg_time, avg_confidence, success_rate * 100.0);
    }
}