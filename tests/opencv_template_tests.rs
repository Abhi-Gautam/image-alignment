...
fn test_ncc_translation_detection() {
    ...
    let expected_y = offset_y as f32 + template.height() as f32 / 2.0 - target.height() as f32 / 2.0;
    
    assert!((result.translation.0 - expected_x).abs() < 2.0, 
           "Translation X should be near {}, got {}", expected_x, result.translation.0);
    assert!((result.translation.1 - expected_y).abs() < 2.0, 
           "Translation Y should be near {}, got {}", expected_y, result.translation.1);
    assert!(result.confidence > 0.7, "Should have high confidence for clear match");
}
...
fn test_noise_robustness() {
    ...
    assert!(result.translation.0.abs() < 3.0, "Should handle noise reasonably");
    assert!(result.translation.1.abs() < 3.0, "Should handle noise reasonably");
    assert!(result.confidence > 0.3, "Should have some confidence even with noise");
}
...
fn test_sub_pixel_accuracy() {
    ...
    assert!((result.translation.0 - 1.0).abs() < 1.5, 
           "Should detect translation close to 1.0, got {}", result.translation.0);
    assert!((result.translation.1 - 1.0).abs() < 1.5, 
           "Should detect translation close to 1.0, got {}", result.translation.1);
}
