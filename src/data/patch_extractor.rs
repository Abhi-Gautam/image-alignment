...
impl PatchExtractor {
    ...
    pub fn extract_patch(
        ...
        if center_x + half_size >= source_image.width()
            || center_y + half_size >= source_image.height()
        {
    ...
    pub fn extract_good_patches_with_config(
        ...
        while patches.len() < count as usize && attempts < max_attempts {
            ...
            let center_x =
                half_size + (rand::random::<u32>() % (source_image.width() - patch_size));
            let center_y =
                half_size + (rand::random::<u32>() % (source_image.height() - patch_size));
        ...
        if patches.is_empty() {
            return Err(anyhow::anyhow!(
                "Could not find any good patches in the image"
            ));
        }
    ...
    pub fn save_patches(
        ...
        for (i, (patch, center_x, center_y)) in patches.iter().enumerate() {
            let filename = format!(
                "{}_patch_{:02}_at_{}x{}.png",
                base_name, i, center_x, center_y
            );
